...
import com.chen.dao.ShopDao;
import com.chen.entity.Shop;
import com.chen.entity.ShopCategory;
import com.chen.exceptions.ShopOperationException;
...
public class ShopServiceImpl implements ShopService {
    ...
    public boolean addShop(Shop shop, FileContainer fileContainer) {
        ...
        int updateShop = shopDao.updateShop(temp);
        if (updateShop <= 0) {
            // 修改店铺图片失败！
    ...
    public boolean modifyShop(Shop shop, FileContainer fileContainer) {
        ...
        if (shopImage != null && fileName != null && !"".equals(fileName)) {
            // 删除原来的图片并将新上传的保存
            Shop temp = shopDao.queryShopByShopId(shop.getShopId());
    ...
    public List<Shop> getShopList(int userId) {
        Shop s = new Shop();
        s.setOwnerId(userId);
        int number = shopDao.queryShopNumber(s);
        return shopDao.queryShopList(s, 0, number);
    }
    ...
    public List<Shop> getShopList(String shopName, Integer shopCategoryId, Integer areaId, Integer rowIndex, Integer pageSize, Integer parentClassNumber) {
        ...
        return shopDao.queryShopList(shop, rowIndex, pageSize);
    }
    ...
    public File getShopImage(int shopId) {
        Shop shop = shopDao.queryShopImageByShopId(shopId);
